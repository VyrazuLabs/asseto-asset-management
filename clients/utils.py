...
def export_clients_csv_utils(request):
    ...
    try:
        ...
        writer.writerow([
            'Client ID', 'Name', 'Industry', 'Contact Person', 'Email', 'Phone',
            'Total Assets', 'Open Tickets', 'Status'
        ])
...
class ClientService:
    ...
    def stats(user):
        ...
        total_active_rentals = client_queryset.aggregate(total=Sum('asset_count'))['total'] or 0
        return {
            ...
            'deleted_client_count': Client.deleted_objects.filter(organization=user.organization).count(),
        }
    ...
    def update(request, client_id: int):
        ...
        client = get_object_or_404(
            Client.undeleted_objects, pk=client_id, organization=request.user.organization
        )
        form = request.form
        client = form.save(commit=False)
    ...
    def get_detail_context(request, client_id: int):
        ...
        assets_qs = (
            ...
            .select_related('product__product_type', 'product__product_sub_category', 'location')
        ...
        assets_page = Paginator(assets_qs, 10, orphans=1).get_page(request.GET.get('asset_page'))
        ...
        for client_history_entry in client_history:
            ...
            action = {'Created': 'Registration', 'Changed': 'Updated'}.get(history_type_display, history_type_display)
            notes = (
                'Initial registration of the client record.' if history_type_display == 'Created' else
                'Client profile information was updated.' if history_type_display == 'Changed' else
                f'Client record {history_type_display.lower()}.'
            )
            activities.append({
        ...
        for asset_history_entry in asset_history:
            ...
            action = 'Updated' if history_type_display == 'Changed' else history_type_display
            activities.append({
