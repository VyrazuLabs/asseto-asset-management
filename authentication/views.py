...
from authentication.utils import create_db_connection, generate_qr
...
def index(request):
    ...
    all_location_list = (
        Location.undeleted_objects.filter(
            Q(organization=None) | Q(organization=request.user.organization)
        )
        .order_by("created_at")
        .reverse()[0:5]
...
def recent_vendors_partial(request):
    ...
    for it in latest_vendor_list:
        it.formatted_date = (
            format_datetime(x=it.created_at, output_format=obj["date_format"])
            if obj["date_format"]
            else it.created_at.date()
        )
    return render(
        request,
        "dashboard/partials/recent_vendors.html",
        {"latest_vendor_list": latest_vendor_list},
    )
...
def recent_products_partial(request):
    ...
    return render(
        request,
        "dashboard/partials/recent_products.html",
        {"latest_product_list": latest_product_list},
    )
...
def recent_locations_partial(request):
    ...
    return render(
        request,
        "dashboard/partials/recent_locations.html",
        {"all_location_list": all_location_list},
    )
...
def recent_users_partial(request):
    ...
    return render(
        request,
        "dashboard/partials/recent_users.html",
        {"latest_users_list": latest_users_list},
    )
        ...
        if form.is_valid():
            ...
            if user is not None:
                if not AssetStatus.objects.filter(
                    can_modify=False
                ).first() or not AssetStatus.objects.filter(name="Available"):
                    AssetStatus.objects.create(
                        name="Available", organization=None, can_modify=False
                    )
                ...
                if get_user.two_factor_auth and get_totp and get_totp.is_validate:
...
def profile(request):
    ...
    context = {
        ...
        "browser_notification": user.browser_notification,
        "slack_notification": user.slack_notification,
        ...
        if user.is_superuser:
            if form.is_valid():
                ...
                if (old_email) != (new_email):
                    ...
                    messages.success(
                        request,
                        "Profile updated successfully. and verification mail has been sent to the new email address.",
                    )
                else:
...
def organization_info_update(request):
    ...
    context = {"organization_form": organization_form, "organization": organization}
