...
def get_department_list(request, page_number=None):
    ...
    base_query = Department.undeleted_objects.filter(
    ...
    deleted_department_count = Department.deleted_objects.filter(
        organization=request.user.organization
    ).count()
    ...
    department_asset_count = {
        item["user__department"]: item["asset_count"] for item in asset_counts
    }
...
def get_product_type_list(request, page_number=None):
    ...
    deleted_product_types_count = ProductType.deleted_objects.filter(
        can_modify=True
    ).count()
    ...
    product_type_asset_count = {
        item["product__product_type"]: item["asset_count"] for item in asset_counts
    }
...
def get_license_type_list(request, page_number=None):
    ...
    license_counts = (
        License.undeleted_objects.filter(license_type__in=license_type_list)
        .values("license_type")
        .annotate(license_count=Count("id", distinct=True))
    )

    license_type_count = {
        item["license_type"]: item["license_count"] for item in license_counts
    }
...
def get_product_category_list(request, page_number=None):
    ...
    # BASE QUERY
    base_query = ProductCategory.undeleted_objects.filter(
        organization=request.user.organization
    )
    ...
    deleted_categories_count = ProductCategory.deleted_objects.filter(
        organization=request.user.organization
    ).count()
    ...
    if search_text:
        base_query = base_query.filter(
            Q(name__icontains=search_text) | Q(parent__name__icontains=search_text)
        )
    ...
    product_category_asset_count = {
        item["product__product_sub_category_id"]: item["count"] for item in asset_counts
    }
...
def get_location_list(request, page_number=None):
    ...
    deleted_location_count = Location.deleted_objects.filter(
        organization=request.user.organization
    ).count()
    ...
    asset_counts = (
        Asset.undeleted_objects.filter(
            organization=request.user.organization, location__in=location_list
        )
        .values("location")
        .annotate(asset_count=Count("id"))
    )
    location_asset_count = {
        item["location"]: item["asset_count"] for item in asset_counts
    }

    # Stats for summary cards
    total_locations = Location.undeleted_objects.filter(
        organization=request.user.organization
    ).count()
    active_locations = Location.undeleted_objects.filter(
        organization=request.user.organization, status=True
    ).count()
    inactive_locations = Location.undeleted_objects.filter(
        organization=request.user.organization, status=False
    ).count()
