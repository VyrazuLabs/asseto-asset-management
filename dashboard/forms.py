...
class LocationForm(forms.ModelForm):
    ...
    contact_person_phone = forms.IntegerField(
        required=False,
...
class ProductTypeForm(forms.ModelForm):
    ...
    def clean_name(self):
        ...
        if (
            ProductType.undeleted_objects.filter(
                name__iexact=name, organization=self._organization
            )
            .exclude(pk=self._pk)
            .exists()
        ):
...
class ProductCategoryForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["parent"].initial = ProductCategory.undeleted_objects.filter(
            name="Root"
        ).first()
