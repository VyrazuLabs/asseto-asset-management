...
class ProductCategoryListForFormDropdown(APIView):
    ...
    def get(self, request):
        try:
            get_product_categories = ProductCategory.undeleted_objects.filter(
                status=True, parent__name="Root"
            )
...
class ProductSubCategoryListForFormDropdown(APIView):
    ...
    def get(self, request):
        ...
        try:
            product_sub_categories = ProductCategory.undeleted_objects.filter(
                parent__id=id, status=True
            )
            if product_sub_categories:
                data = [
                    {"id": product_sub_category.id, "name": product_sub_category.name}
