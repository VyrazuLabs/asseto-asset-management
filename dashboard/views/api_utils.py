...
def convert_asset_to_list(request, queryset):
    ...
    for asset in queryset:
        ...
        asset_dict["image"] = (
            f"http://{current_host}" + assetImage.image.url if assetImage else None
        )
        if asset.is_assigned:
            assigned_asset = (
                AssignAsset.objects.filter(asset=asset.id)
                .select_related("user")
                .first()
            )
            asset_dict["assigned_to_name"] = (
                assigned_asset.user.full_name
...
def search_utils(request, search_text):
    ...
    response_data = {"products": [], "assets": [], "users": []}

    # -------- Products --------
    products = (
        Product.undeleted_objects.filter(org_filter & Q(name__icontains=search_text))
        .annotate(
    ...
    assets = Asset.undeleted_objects.filter(
        org_filter & (Q(tag__icontains=search_text) | Q(name__icontains=search_text))
    ).order_by("-created_at")[:10]
    ...
    if request.user.is_superuser:
        users = (
            User.undeleted_objects.filter(
                org_filter & Q(is_superuser=False) & Q(full_name__icontains=search_text)
            )
