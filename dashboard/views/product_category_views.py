...
def product_category_list(request):
    ...
    page_object, product_category_asset_count, stats = get_product_category_list(
        request, page_number
    )
...
def add_product_category(request):
    ...
    if request.method == "POST":
        form = ProductCategoryForm(request.POST, organization=request.user.organization)
    ...
    return render(
        request, "dashboard/product_category/product-category-modal.html", context
    )
...
def product_category_details(request, id):
    ...
    product_category = get_object_or_404(ProductCategory.undeleted_objects, pk=id)
    ...
    context = {
        ...
        "title": f"Details-{product_category.name}",
    }
...
def delete_product_category(request, id):
    ...
    if request.method == "POST":
        ...
        assigned_assets = AssignAsset.objects.filter(
            asset__product__product_sub_category=product_category
        ).first()
        if assigned_assets is not None:
            messages.error(
                request,
                "Product Category cannot be deleted as it is assigned to an asset. Please unassign the asset before deleting the product category.",
            )
...
def search_product_category(request, page):
    ...
    return render(
        request,
        "dashboard/product_category/product-categories-data.html",
        {
            "page_object": page_object,
            "product_category_asset_count": product_category_asset_count,
            "total_categories": stats["total_categories"],
