...
def product_type_list(request):
    page_object, product_type_form, product_type_asset_count, stats = (
        get_product_type_list(request)
    )
...
def add_product_type(request):
    ...
    return render(
        request,
        "dashboard/product_type/product-type-modal.html",
        {
            "form": form,
            "is_add_mode": True,
        },
...
def delete_product_type(request, id):
    if request.method == "POST":
        ...
        if assigned_assets:
            messages.error(
                request, "Product Type cannot be deleted as it is assigned to an asset."
            )
...
def search_product_type(request, page):
    page_object, _, product_type_asset_count, _ = get_product_type_list(
        request, page_number=page
    )

    return render(
        request,
        "dashboard/product_type/product-types-data.html",
        {
            "page_object": page_object,
            "product_type_asset_count": product_type_asset_count,
        },
