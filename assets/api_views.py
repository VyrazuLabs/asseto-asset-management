...
class GetNotifications(APIView):
    ...
    def get(self, request):
        try:
            in_app_notifications_status = (
                True if request.user.inapp_notification else False
            )
    ...
    def patch(self, request):
        try:
            ...
            if not notification_id:
                return api_response(
                    status=400, error_message="notification_id is required"
                )

            mark_notification_as_seen(notification_id, request)

            return api_response(message="Notification marked as seen successfully")
...
class AssetList(APIView):
    ...
    def get(self, request):
        try:
            asset_queryset = (
                Asset.undeleted_objects.select_related(
                    "vendor",
                    "product",
...
class UpdateAsset(APIView):
    ...
    def patch(self, request, id):
        ...
        try:
            asset_data = AssetSerializer(
                get_asset, data=request.data, context={"request": request}, partial=True
            )
...
class Scan_api_barcode(APIView):
    @extend_schema(
        description="for the tag id, the variable name from frontend must be 'tag_id' "
    )
    ...
    def post(self, request, id):
        try:
            ...
            if get_asset.is_assigned == True:
                return api_response(
                    status=400, message="this asset is already assigned"
                )
...
class UserListForAssignAsset(APIView):
    ...
    def get(self, request):
        try:
            ...
            return api_response(
                data=data, message="users for assign asset get successfully"
            )
...
class GetWarrantyExpiredAssetFlag(APIView):
    ...
    def get(self, request):
        try:
            user = request.user
            data = {
                "use_warranty_expired_assets": (
                    True if user.use_expired_assets else False
                )
            }
