...
def assign_asset_user_list(request):
    get_users = User.undeleted_objects.filter(
        status=True, organization=request.user.organization
...
def convert_to_list(request, queryset):
    ...
    assignments_qs = AssignAsset.objects.select_related("user").filter(
    ...
    for asset in queryset:
        ...
        asset_dict["image"] = (
            f"http://{current_host}" + asset_img.image.url
            if asset_img and asset_img.image
            else None
        )

        if asset.is_assigned:
            assigned_asset = asset_assign_map.get(asset.id)
            asset_dict["assigned_to_name"] = (
                dynamic_display_name(request, fullname=assigned_asset.user.full_name)
                if assigned_asset and assigned_asset.user
                else None
            )
            asset_dict["assigned_to_image"] = (
                f"http://{current_host}" + assigned_asset.user.profile_pic.url
                if assigned_asset
                and assigned_asset.user
                and assigned_asset.user.profile_pic
                else None
            )
        else:
...
def get_assigned_user(request, asset):
    ...
    if asset.is_assigned:
        assigned_asset = (
            AssignAsset.objects.filter(asset=asset).select_related("user").first()
        )
        if assigned_asset:
            assigned_user = {
                ...
                "full_name": (
                    dynamic_display_name(
                        request, fullname=assigned_asset.user.full_name
                    )
                    if assigned_asset and assigned_asset.user
                    else None
                ),
...
def asset_data(request, asset, asset_images):
    ...
    asset_data = {
        ...
        "product_category": (
            asset.product.product_sub_category.name
            if asset.product.product_sub_category
            else None
        ),
        "serial_no": asset.serial_no if asset.serial_no else None,
        "price": (
            format_currency + " " + str(asset.price) if asset.price else None
        ),  # asset.price if asset.price else None,
        ...
        "purchase_date": (
            format_datetime(asset.purchase_date, format_date)
        ...
        "warranty_expiry_date": (
            format_datetime(asset.warranty_expiry_date, format_date)
        ...
        "original_warranty_expiry_date": (
            asset.warranty_expiry_date.isoformat()
            if asset.warranty_expiry_date
            else None
        ),
        "vendor_id": asset.vendor.id if asset.vendor else None,
    ...
    if asset_images:
        for image in asset_images:
            asset_images_dict = {
                "image_id": image.id,
                "image_path": f"http://{current_host}" + image.image.url,
            }
...
def get_asset_id(tag_id):
    ...
    if get_tag:
        ...
        respones_data = {"id": asset_id.id, "status": "success"}
...
def get_asset(tag_id):
    ...
    get_assign_asset = (
        AssignAsset.objects.filter(asset=asset).first() if asset else None
    )
    if not asset:
    ...
    return (
        {
            "asset_id": asset.id,
            "assigned_to_id": get_assign_asset.id,
            "assigned_to_name": get_assign_asset.user.full_name,
        }
        if get_assign_asset
        else {"asset_id": asset.id, "assigned_to_id": None, "assigned_to_name": None}
    )
...
def convert_svg_mm_to_px(svg):
    ...
    svg = re.sub(
        r'width="([\d\.]+)mm"', lambda m: f'width="{mm_to_px(m.group(1))}px"', svg
    )
    svg = re.sub(
        r'height="([\d\.]+)mm"', lambda m: f'height="{mm_to_px(m.group(1))}px"', svg
    )
