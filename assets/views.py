...
from .forms import (
    ...
    AssetImageForm, ReassignedAssetForm,
)
from .models import Asset, AssetImage, AssetSpecification, AssetStatus, AssignAsset
from .utils import (
    asset_details, autogenerated_tag, create_char_data, create_custom_fileds,
    delete_asset_images, filtered_asset, create_asset_list, save_new_images,
    search_with_filters, update_existing_custom_fields,
    get_asset_images, get_audit_images, delete_assign_asset, search_asset,
    add_asset, assign_asset_in_form, details_of_asset, slack_notification,
)
from .services import change_asset_status, unassign_asset_from_list, assign_asset_to_user
...
def assigned_list(request):
    assign_asset_list = AssignAsset.objects.select_related(
        'asset', 'asset__vendor', 'asset__product', 'asset__location', 'asset__asset_status', 'user', 'user__department'
    ).filter(
...
def piechart_status_data(request):
    data = Asset.undeleted_objects.filter(
        organization=request.user.organization
    ).values('asset_status__name').annotate(total=Count('id'))
    return data
...
def assign_asset_search(request, page):
    ...
    if search_text:
        return render(request, 'assets/assigned-assets-data.html', {
            ...
            ).filter(Q(asset__organization=request.user.organization) & (Q(
    ...
    asset_list = AssignAsset.objects.select_related(
        'asset', 'user', 'user__department'
    ).filter(
