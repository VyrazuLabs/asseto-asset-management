...
from gate_pass.serializers import GatePassCreateSerializer, SearchGatePassSerializer
from gate_pass.utils import (
    GatePassFilterService,
...
class GatePassSearch(APIView):
    ...
    @extend_schema(
        description='from frontend the search field name must be "search_text"',
        parameters=[
            OpenApiParameter(
                name="search_text",
    ...
    def get(self, request):
        try:
            ...
            queryset = self.filter_service.apply_filters(
                self.repository.get_all(), request.GET
            )

            data = [
                GatePassSerializer.serialize_list_item(item, request)
...
class GatePassCreate(APIView):
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            serializer.save()
            return api_response(
                data=serializer.data, message="GatePass created successfully"
            )
...
class GatePassApprove(APIView):
    ...
    @extend_schema(
        description='from frontend name must be "id"',
        parameters=[
            OpenApiParameter(
                name="id",
                type=OpenApiTypes.STR,
