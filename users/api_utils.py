...
def get_username(request, id):
    ...
    dict = {"full_name": get_name, "profile_pic": f"{host}{get_profile_img}"}
    arr.append(dict)
    return arr
...
def get_profile_data(request):
    ...
    dict = {
        ...
        "contact_person_name": (
            user.department.contact_person_name if user.department else None
        ),
        "contact_person_phone": (
            user.department.contact_person_phone if user.department else None
        ),
        "contact_person_email": (
            user.department.contact_person_email if user.department else None
        ),
        "organization_name": (
            request.user.organization.name if request.user.organization else None
        ),
        "organization_website": (
            request.user.organization.website
            if request.user.organization.website
            else None
        ),
        "organization_email": (
            request.user.organization.email if request.user.organization.email else None
        ),
        "organization_phone": (
            request.user.organization.phone if request.user.organization.phone else None
        ),
...
def get_roles(request):
    try:
        ...
        for role in roles_list:
            role_dict = {"id": str(role.id), "name": role.related_name}
