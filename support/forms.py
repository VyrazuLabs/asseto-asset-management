...
class SupportTicketForm(forms.ModelForm):
    ...
    priority = forms.ChoiceField(
        choices=PRIORITY_CHOICES, widget=forms.HiddenInput(), initial="1"
    )
    ticket_type = forms.ChoiceField(
        choices=TICKET_TYPE_CHOICES, widget=Select(attrs={"class": "form-control"})
    )
    ...
    hours_worked = forms.DecimalField(
        initial=0.0, widget=NumberInput(attrs={"class": "form-control", "step": "0.1"})
    )
    impact_level = forms.ChoiceField(
        choices=IMPACT_CHOICES, widget=forms.HiddenInput(), initial="1"
    )
    status = forms.ChoiceField(
        choices=STATUS_CHOICES, widget=forms.HiddenInput(), initial="0"
    )
...
class TicketNoteForm(forms.Form):
    ...
    is_internal = forms.BooleanField(
        required=False, label="Mark as Internal Note (hidden from client)"
    )
