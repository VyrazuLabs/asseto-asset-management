...
def _ticket_base_qs(request):
    ...
    return (
        SupportTicket.undeleted_objects.filter(organization=request.user.organization)
...
def search_tickets(request, page):
    ...
    return render(
        request,
        "support/tickets-data.html",
        {"page_object": page_object, "asset_images": asset_images},
    )
...
def technician_search(request):
    query = request.GET.get("q", "").strip()
    if not query:
        users = User.objects.filter(organization=request.user.organization).order_by(
            "full_name"
        )[:10]
    else:
