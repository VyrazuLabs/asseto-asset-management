...
from configurations.api_utils import (
    get_tag_configurations,
    get_localization_configurations,
)
...
class AutoGeneratedTag(APIView):
    def get(self, request):
        ...
        try:
            ...
            if tag_config:
                auto_generated_tag = generate_asset_tag(
                    prefix=tag_config.prefix, number_suffix=tag_config.number_suffix
                )
            else:
                auto_generated_tag = generate_asset_tag(
                    prefix="VY", number_suffix="001"
                )
            data = {"auto_generated_tag": auto_generated_tag}

            return api_response(
                data=data, message="Auto Generated tag fetched successfully"
            )
        except Exception as e:
