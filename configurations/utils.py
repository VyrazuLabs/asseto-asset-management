...
def process_uploaded_logos(request, **files):
    Uploaded = {}
    for fields, file in files.items():
        if file:
            Uploaded[fields] = Upload.image_upload(request, file, fields)
    return Uploaded
...
def add_path(organization):
    ...
    if brand_img:
        ...
        if brand_img.login_page_logo:
            image_dist["login_page_logo"] = (
                f"{brand_img.login_page_logo_path}{brand_img.login_page_logo}"
            )
    return image_dist


def create_or_update_image(
    request, logo, favicon, login_page_logo, file_dist, organization
):
    try:
        if any(
            [
                file_dist.get("logo"),
                file_dist.get("favicon"),
                file_dist.get("login_page_logo"),
            ]
        ):
...
def get_currency_and_datetime_format(organization):
    getLocalization = LocalizationConfiguration.objects.filter(
