...
from vendors.utils import (
    ...
    searched_data,
    vendor_details,
    vendor_list_for_form,
    get_assigned_asset_by_vendor,
)
...
class VendorList(APIView):
    ...
    def get(self, request):
        try:
            vendor_queryset = Vendor.undeleted_objects.filter(
            ...
            return api_response(
                data=paginated_data, message="Vendor list get successfully"
            )
    ...
    def delete(request, self, id):
        try:
            ...
            if get_assigned_asset_by_vendor_list:
                return api_response(
                    status=400,
                    error_message="Vendor has assigned assets and could not be deleted",
                )
            get_vendor = get_object_or_404(Vendor, pk=id)
