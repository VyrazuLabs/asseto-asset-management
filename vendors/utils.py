...
def export_vendors_pdf_utils(request):
    ...
    response["Content-Disposition"] = (
        f'attachment; filename="export-vendors-{today}.pdf"'
    )
    return response
...
def export_vendor_csv_utils(request):
    ...
    vendors_list = (
        Vendor.undeleted_objects.filter(organization=request.user.organization)
        .order_by("-created_at")
        .values_list(
            "name",
            "email",
    ...
    response["Content-Disposition"] = (
        f'attachment; filename="export-vendors-{today}.csv"'
    )
    return response
...
def search_utils(request, page):
    ...
    filters = Q(organization=None) | Q(organization=request.user.organization)
    ...
    deleted_vendor_count = Vendor.deleted_objects.filter(
        organization=request.user.organization
    ).count()
...
def vendor_list_util(request):
    ...
    for vendor in page_object:
        count_array.append(get_count_of_assets(request, vendor.id))
...
def get_count_of_assets(request, vendor_id):
    ...
    """
    get_count = Asset.objects.filter(
        organization=request.user.organization, vendor=vendor_id, is_deleted=False
    ).count()
    return get_count
...
def vendor_details(get_vendor, request):
    vendor_details_dict = {
        ...
        "contact_person": (
            get_vendor.contact_person if get_vendor.contact_person else None
        ),
        "gstin_number": get_vendor.gstin_number if get_vendor.gstin_number else None,
        "address": (
            get_vendor.address.address_line_one
            if get_vendor.address and get_vendor.address.address_line_one
            else None
        ),
...
def vendor_list_for_form(vendors):
    ...
    for vendor in vendors:
        vendor_dict = {"id": vendor.id, "name": vendor.name}
