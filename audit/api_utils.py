...
def get_completed_audit(request):
    ...
    audit_queryset = Audit.objects.filter(created_at__gte=thirty_days_ago).order_by(
        "-created_at"
    )
...
def get_completed_audits(request, audit_queryset):
    ...
    for it in audit_queryset:
        dict = {
            ...
            "audited_by": (
                it.audited_by.dynamic_display_name(request.user.full_name)
                if it.audited_by
                else ""
            ),
            ...
            "notes": it.notes,
            "next_audit_date": (
                format_datetime(next_audit_due(it.id), format_date)
                if next_audit_due(it.id)
                else ""
            ),
            "assigned_to": (
                dynamic_display_name(request, fullname=it.assigned_to)
                if it.assigned_to
                else ""
            ),
...
def get_pending_audits(request):
    ...
    for asset in asset_list:
        ...
        dict = {
            ...
            "asset_image_url": (
                f"http://{current_host}" + get_asset_img.image.url
                if get_asset_img
                else ""
            ),
            ...
            "product_category": (
                ...
                if asset and asset.product and asset.product.product_sub_category
                else ""
            ),
            "asset_status": asset.status if asset else "",
            "expected_audit_date": (
                format_datetime(next_due_date, format_date) if next_due_date else ""
            ),
            "last_audit_date": (
                format_datetime(has_audit.created_at.date(), format_date)
                if has_audit
                else ""
            ),
