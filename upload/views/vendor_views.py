...
def search_vendor_upload(request, page):
    ...
    return render(
        request,
        "upload/vendor-upload-data.html",
        {
            "page_object": page_object,
        },
    )
...
def import_vendors_csv(request):
    if request.method == "POST":
        ...
        return render(
            ...
            {"page": "Vendors", "hx_target": "#mapping-vendors-modal-content"},
        )
...
def vendor_render_to_mapper_modal(request):
    ...
    if request.method == "POST":
        ...
        messages.success(
            request, f"{len(created_vendors)} vendors imported successfully."
        )
        return redirect("upload:vendor_list")
    ...
    if request.method == "POST":
        try:
            ...
            for it in data:
                ...
                get_user = Vendor.objects.filter(
                    email=it.get("email"),
                    full_name=it.get("first_name"),
                    phone=it.get("phone"),
                    contact_person=it.get("contact_person"),
                    gstin_number=it.get("gstin_number"),
