...
from upload.utils import render_to_csv, csv_file_upload, get_product_type_upload_list
import pandas as pd
...
@login_required
def search_product_type_upload(request, page):
    page_object, _ = get_product_type_upload_list(request, page_number=page)
    return render(
        request,
        "upload/product-type-upload-data.html",
        {
            "page_object": page_object,
        },
    )
...
def export_product_types_csv(request):
    ...
    response["Content-Disposition"] = (
        f'attachment; filename="sample-product-types-file.csv"'
    )
    return response
...
def import_product_types_csv(request):
    ...
    if request.method == "POST":
        ...
        return render(
            request,
            "upload/map-product-type-modal.html",
        ...
        return render(
            ...
            {
                "page": "Product Type",
                "hx_target": "#upload-product-types-modal-content",
            },
        )
...
def product_type_render_to_mapper_model(request):
    if request.method == "POST":
        ...
        messages.success(
            request,
            f"{len( created_product_types)} Product Types imported successfully.",
        )
        return redirect("upload:product_type_list")
        ...
        try:
            ...
            for it in data:
                ...
                obj = ImportedUser.objects.create(
                    entity_type="ProductType", name=it.get("name")
                )
