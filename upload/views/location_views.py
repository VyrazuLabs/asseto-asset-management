...
def search_location_upload(request, page):
    ...
    return render(
        request,
        "upload/location-upload-data.html",
        {
            "page_object": page_object,
        },
    )
...
def export_locations_csv(request):
    ...
    response["Content-Disposition"] = (
        f'attachment; filename="sample-locations-file.csv"'
    )
    return response
...
def import_locations_csv(request):
    if request.method == "POST":
        ...
        return render(
            request,
            "upload/map-location-modal.html",
            {
                "headers": headers,
                "fields": [
                    "Name",
        ...
        return render(
            ...
            {"page": "Locations ", "hx_target": "#upload-locations-modal-content"},
        )
...
def location_render_to_mapper_modal(request):
    if request.method == "POST":
        ...
        messages.success(
            request, f"{len(created_location)} location imported successfully."
        )
        return redirect("upload:location_list")
