...
from upload.utils import render_to_csv, csv_file_upload, get_department_upload_list
import pandas as pd
...
@login_required
def search_department_upload(request, page):
    page_object, _ = get_department_upload_list(request, page_number=page)
    return render(
        request,
        "upload/department-upload-data.html",
        {
            "page_object": page_object,
        },
    )
...
def export_departments_csv(request):
    ...
    response["Content-Disposition"] = (
        f'attachment; filename="sample-departments-file.csv"'
    )
    return response
...
def import_departments_csv(request):
    if request.method == "POST":
        ...
        return render(
            request,
            "upload/map-department-modal.html",
            {
                "headers": headers,
                "fields": [
                    "Department Name",
        ...
        return render(
            ...
            {"page": "Department", "hx_target": "#upload-departments-modal-content"},
        )
...
def department_render_to_mapper_modal(request):
    if request.method == "POST":
        ...
        messages.success(
            request, f"{len(created_departments)} Departments imported successfully."
        )
        return redirect("upload:department_list")
        ...
        try:
            ...
            for it in data:
                ...
                obj = ImportedUser.objects.create(
                    ...
                    name=it.get("name"),
                    phone=it.get("phone"),
                    contact_person_name=it.get("contact_person_name"),
                    contact_person_email=it.get("contact_person_email"),
                    contact_person_phone=it.get("contact_person_phone"),
                )
