...
def client_portal_logout(request):
    ...
    keys_to_remove = ['client_contact_id', 'client_id', 'pending_contact_id', 'pending_contact_email']
...
def client_portal_dashboard(request):
    ...
    recent_asset_images = {}
    if recent_asset_ids:
        for img in AssetImage.objects.filter(asset_id__in=recent_asset_ids).order_by('-uploaded_at'):
    ...
    cat_data = {}
    cat_colors = ['#435ebe', '#198754', '#ffc107', '#dc3545', '#0dcaf0', '#6f42c1', '#fd7e14', '#20c997']
...
def _client_ticket_stats(client):
    ...
    return {
        ...
        'overdue_count': qs.filter(estimated_eta__lt=timezone.now()).exclude(status__in=['3', '4']).count(),
        'critical_count': qs.filter(priority='3').exclude(status__in=['3', '4']).count(),
    }
...
def client_portal_export_support_tickets(request):
    ...
    writer.writerow([
        'Ticket ID', 'Subject', 'Asset', 'Priority', 'Status',
        'Ticket Type', 'Assigned To', 'Created At'
    ])
